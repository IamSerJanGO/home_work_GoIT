...
def create_folders(my_path, folder_list=sorting_folders):  # Функция создает папки для сортировки
    for folder in folder_list:
        folder_path = Path(my_path) / folder
        folder_path.mkdir(exist_ok=True)


def dearchives_func(file_name, my_path):  # Функция разархивирущая архивы в папки
    path_to_unpack = f'{my_path}/archives/'
